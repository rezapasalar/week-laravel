...
class ContactChart extends Component
{
    public $users = 0;
    ...
    {
        $this->users = Contact::whereNull('parent_id')->count();

        $this->managers = Contact::whereNotNull('parent_id')->count();
    }
    ...
    public function render()
    {
        return <<<'blade'
            <div class="col-span-4 md:col-span-1 py-4">
                ...
                <canvas id="myContactChart" height="100"></canvas>

                <script>
                    document.addEventListener('DOMContentLoaded', () => {
            
                        const ctx = document.getElementById('myContactChart').getContext('2d');
            
                        Chart.defaults.font.family = 'Vazir';
            
                        const myContactChart = new Chart(ctx, {
                            type: 'doughnut',
                            data: {
                                labels: ['کاربران', 'مدیریت'],
                                datasets: [{
                                    label: '# of Votes',
                                    data: [@this.users, @this.managers],
                                    backgroundColor: [
                                        '#0d47a1',
                                        '#bbdefb',
                                    ],
