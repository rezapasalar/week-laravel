<div>
    <x-jet-dialog-modal wire:model="editImageModal">
        <x-slot name="title">
            ویرایش تصویر
        </x-slot>
        ...
        <x-slot name="content">
            ...
            <div class="pb-2" x-data="{photoPreview: ''}" x-init="
                window.addEventListener('close:editPhotoSlideshow', () => {
                    photoPreview = '';
        ...
        <x-slot name="footer">
            <div class="text-left space-x-reverse space-x-2">
                ...
                <x-jet-button wire:click="edit" wire:loading.attr="disabled" wire:target="photo">
                    ویرایش
                </x-jet-button>
            </div>
        </x-slot>
    ...
    <script>
        window.addEventListener('edit:modal', (e) => @this.open(e.detail.filename));
    </script>
</div>