<x-admin.global.form-modal type="edit" wireTarget="photo" subject="product" description="برای ثبت محصول به نکاتی دقت کنید, وزن محصول را حتما به گرم نوشته, قیمت محصول را به ریال ثبت کرده و عرض تصویر محصول 550 پیکسل و ارتفاع محصول 420 میباشد.">
    ...
    <div class="mb-4" x-data="{photoPreview: null}" x-init="
        window.addEventListener('close:editProduct', () => {
            photoPreview = null;
            $refs.photo.files = [];
        })
    ">
        <x-jet-label>تصویر</x-jet-label>
        <x-jet-input x-ref="photo" type="file" class="hidden" wire:model="photo"
                     x-on:change="
        ...
        <div class="mt-3 flex justify-center" x-show="!photoPreview">
            <span id="photoProductFileReander" class="block w-40 h-40 bg-cover bg-no-repeat bg-center" style="background-image: url({{ asset('storage/product-photos/' . $product->photo_path) }})"></span>
        </div>
